...
将获取的片段放入对应的 `mapper.xml` 中，按需补充表关联（JOIN）或自定义语句：
...
<select id="selectByXml" resultType="com.example.test.vo.SysUserVO">
    ...
    <where>
        ...
        <!-- 追加自定义条件（未映射的参数可通过 param1.extra 获取） -->
        AND a.deleted = 0
...
// 继承 MpMapper 即可 (MpMapper 内部已继承 BaseMapper)
